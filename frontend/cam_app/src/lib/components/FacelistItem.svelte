...
{#if details.status.includes("FR") }

    <div class="bg-wgray-50 mt-1 mb-2 shadow-sm border-2 border-red-700 rounded-md">

        <div class="mt-0 flex bg-red-200">
    ...
    <div class="bg-bgray-50 mt-1 mb-2 shadow-sm   border border-green-700 rounded-md">
        <div class="mt-0 flex ">
            <img class="mt-0 w-52 h-52 object-center"
                 src={recognized_res.identity.face.images.expanded}/>

            <div class="ml-10  mt-2 ">
                <h1 class="uppercase font-semibold text-4xl
                    text-green-800 ">{details.name}</h1>

                <div class="mt-6 text-3xl
                        font-extrabold
                        text-bgray-800 tracking-wide">{confid}
                </div>
                <div class="mt-8 text-2xl font-semibold text-green-700 rounded-md"  >
                    {details.status}
                </div>
            </div>
        </div>
