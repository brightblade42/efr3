<script >
  ...
  async function handle_login() {
	  console.log("handling the login");
	  await auth_store.login(user_name, password);
  }
  let login_failed = false;
  let is_login_disabled = true;
  let in_flight = false;
  $: in_flight = $auth_store.type === "InFlight";
  $: is_login_disabled = ($auth_store.type === "InFlight" || (user_name.length < 2 || password.length < 5))
  $: login_failed = ($auth_store.type === "Failed");

  $: {
	  console.log($auth_store.type);
	  if ($auth_store.type === "LoggedIn") {
		  console.log("watsup");
		  appStore.connect_server();
		  goto('/');
	  }
  }
