...
export class DB {
  ...
  async get_camera(id: number): Promise<Result<CameraData, Error>> {
    ...
    try {
      res = await this.sql<CameraData[]>`
                 ...
                 min_match from eyefr.camera  where id = ${id}`;
  ...
  async get_camera_by_name(name: string): Promise<Result<CameraData, Error>> {
    ...
    try {
      res = await this.sql<CameraData[]>`
                 select id, name, rtsp_url,
                 user_name , password, direction, fr_stream_settings,
                 min_match from eyefr.camera  where name = ${name}`;
  ...
  async get_cameras(): Promise<CameraData[]> {
    ...
    try {
      res = await this.sql<
        CameraData[]
      >`select id, name, rtsp_url,  user_name , password, direction, fr_stream_settings, min_match from eyefr.camera  order by feed_position`;
    } catch (e) {
  ...
  async camera_exists(name: string): Promise<boolean> {
    const res = await this.sql<
      CameraData[]
    >`select id from eyefr.camera where name=${name}`;
    return res.length > 0;
  }
  ...
  async get_last_id(): Promise<number> {
    const res = await this.sql<
      CameraData[]
    >`select id from eyefr.camera order by id desc limit 1`;
    if (res.length > 0) {
      return res[0].id || 0;
    }
    return 0;
  ...
  async delete_camera(name: string): Promise<CameraData[]> {
    const res = await this.sql<CameraData[]>`
            delete from eyefr.camera where name=${name} returning *`;
    return res;
  }
  ...
  async log_error(params: LogErrorOptions): Promise<void> {
    ...
    try {
      ...
      //       insert into eyefr.error_logs (level, kind, data, extra, summary) values (${level}, ${kind}, ${data}, ${extra}, ${summary})
      //       returning id `;
      //don't keep this
      //console.log("error logged id", id_res);
    } catch (e) {
      console.error(
        `${new Date().toLocaleString()} - error logging error: ${kind} : ${summary} : `,
        e,
      );
  ...
  async log_detected_frame(cam: CameraData, dres: DetectedResult) {
    try {
      const _res = await this.sql`
                insert into eyefr.detection_log
  ...
  async update_camera(cam: CameraData): Promise<Result<CameraData[], Error>> {
    try {
      ...
      const res = await this.sql`
              update eyefr.camera set ${this.sql(
                cam,
                "name",
                "rtsp_url",
                "user_name",
  ...
  async save_new_camera(
    camera: CameraData,
  ): Promise<Result<CameraData, Error>> {
    try {
